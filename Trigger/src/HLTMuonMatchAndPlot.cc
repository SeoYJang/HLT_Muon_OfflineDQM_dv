 ...
 */
 ...
 void HLTMuonMatchAndPlot::beginRun(DQMStore::IBooker& iBooker, const edm::Run& iRun, const edm::EventSetup& iSetup) {
  ...
  for (const auto& suffix : EFFICIENCY_SUFFIXES) {
    ...
    if (isTnPAnalyzer) {
      book1D(iBooker, "efficiencyEtatnp_" + suffix, "eta", ";#eta;");
      book1D(iBooker, "efficiencyPttnp_" + suffix, "pt", ";p_{T};");
      book1D(iBooker, "efficiencyPtTag_" + suffix, "pt", ";p_{T};");
    }
    ...
    // Book TnP plots only if this IS a TnP analyzer
    if (isTnPAnalyzer) {
      book1D(iBooker, "efficiencyOfflineMuonPt_" + suffix, "pt", ";p_{T};");
      book1D(iBooker, "efficiencyOfflineMuonEta_" + suffix, "eta", ";#eta;");
      book1D(iBooker, "efficiencyOfflineMuonPhi_" + suffix, "phi", ";#phi;");
    }
  }
 ...
 void HLTMuonMatchAndPlot::analyze(Handle<MuonCollection>& allMuons,
  ...
  for (size_t i = 0; i < targetMuons.size(); i++) {
    ...
    // Fill offline muon pt eta phi distributions (TnP plots only)
    if (isLastFilter_ && isTnPAnalyzer) {
      hists_["efficiencyOfflineMuonPt_denom"]->Fill(muon.pt());
      hists_["efficiencyOfflineMuonEta_denom"]->Fill(muon.eta());
      hists_["efficiencyOfflineMuonPhi_denom"]->Fill(muon.phi());

      // Same plot, just to keep plots format as efficiency plots
      hists_["efficiencyOfflineMuonPt_numer"]->Fill(muon.pt());
      hists_["efficiencyOfflineMuonEta_numer"]->Fill(muon.eta());
      hists_["efficiencyOfflineMuonPhi_numer"]->Fill(muon.phi());
    }
  ...
  // Tag and Probe method for measuring HLT efficiency (TnP analyzer only)
  if (!isLastFilter_)
    return;
  ...
  if (isTnPAnalyzer) {
    for (size_t i = 0; i < targetMuons.size(); ++i) {
      ...
      hists_["efficiencyPtTag_denom"]->Fill(tagCandidate.pt());
      ...
      hists_["efficiencyPtTag_numer"]->Fill(tagCandidate.pt());
      ...
      for (size_t j = 0; j < targetMuons.size(); ++j) {
        ...
        std::string denomSuffix = "tnp_denom";
        hists_["efficiencyEta" + denomSuffix]->Fill(probeCandidate.eta());
        hists_["efficiencyPt" + denomSuffix]->Fill(probeCandidate.pt());
        
        // Check if probe muon passes HLT
        if (matches[j] >= targetMuons.size()) {
          continue;
        }
        
        // Fill numerator histograms
        std::string numerSuffix = "tnp_numer";
        hists_["efficiencyEta" + numerSuffix]->Fill(probeCandidate.eta());
        hists_["efficiencyPt" + numerSuffix]->Fill(probeCandidate.pt());
